...
function Audio() {
  ...
  return (
    <eventbox
      ...
      onScroll={(_, e) =>
        speaker.set_volume(
          e.delta_y < 0 ? speaker.volume + 0.05 : speaker.volume - 0.05,
        )
      }
    >
      <label
...
function Music() {
  const p = Mpris.Player.new("YoutubeMusic");
  ...
  const toggleloop = (loop_status: AstalMpris.Loop) => {
    if (loop_status === AstalMpris.Loop.UNSUPPORTED)
  ...
  const pbstatus = bind(p, "playback_status").as((v) =>
    v === AstalMpris.PlaybackStatus.PLAYING ? " " : " ",
  );
  ...
  return (
    <box>
      {bind(p, "available").as((available) => {
        if (!available) return <label className="purple" label="~" />;
        return (
          <eventbox
            cursor="pointer"
            className={bind(p, "playback_status").as(
              (pbs) =>
                `music ${pbs === AstalMpris.PlaybackStatus.PLAYING ? "purple" : "orange"}`,
            )}
            onClick={(_, e) =>
              e.button === Astal.MouseButton.PRIMARY
                ? p.play_pause()
                : p.set_loop_status(toggleloop(p.loop_status))
            }
...
function Workspaces() {
  ...
  const occupied = Variable.derive(
    [
    ...
    (cls, wss, _) => (i: number) =>
      cls.some((c) => c.workspace.id === i) ||
      wss.find((ws) => ws.id === i)?.clients.length! > 0,
  );
    ...
    <eventbox className="workspaces">
      <box>
        {Array.from({ length: 9 }, (_, i) => i + 1).map((i) => (
          <eventbox onClick={() => dispatch(String(i))} cursor="pointer">
            <box
              className={bind(hyprland, "focusedWorkspace").as((active) =>
                active.id == i ? "workspace-entry current" : "workspace-entry",
              )}
            >
              <label
                label={bind(occupied).as((occ) => (occ(i) ? " ◆ " : " ◇ "))}
                className="workspace"
              />
              <label
                label={i == 3 || i == 6 ? " | " : "  "}
